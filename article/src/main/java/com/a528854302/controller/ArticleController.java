   ...
   @GetMapping("/select")
    ...
    public ResponseResult update(@RequestBody Article article) {
        if (articleService.update(article)) {
            ...
            return new ResponseResult(false,StatusCode.ERROR,"修改失败");
        }
    }
    ...
    public ResponseResult delete(@PathVariable String id) {
        if (articleService.deleteById(id)) {
            ...
            return new ResponseResult(false,StatusCode.ERROR,"删除失败");
        }
    }

    @GetMapping(value="/page/{page}/{size}")
    public ResponseResult getByPage(@RequestBody Map map, @PathVariable int page, @PathVariable int size) {
